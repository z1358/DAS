...
class TTAMethod(nn.Module):
    def __init__(self, cfg, model, num_classes):
        ...
        if "224" in self.cfg.MODEL.ARCH:
            self.img_size = (224, 224)
        elif "384" in self.cfg.MODEL.ARCH:
        ...
        if "vit" in self.cfg.MODEL.ARCH or "VIT" in self.cfg.MODEL.ARCH:
            self.params, param_names = self.collect_params_vit()
        else:
        ...
        if self.params is not None:
            ...
            self.optimizer = None
            self.num_trainable_params, self.num_total_params = 0, 0
