...
class OBAO(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        if "cifar" in self.dataset_name and 'vit' not in self.cfg.MODEL.ARCH.lower():
            input_size = [3, 32, 32]
        else:
            input_size = [3, 224, 224]
        self.buffer = Buffer(self.cfg, input_size, self.num_classes, cfg.MIXED_PRECISION).cuda()
        ...
        if self.dataset_name == "domainnet126":
            ...
            safe_arch_name = arch_name.replace(" ", "_").replace(".", "_")
            fname = f"protos_{self.dataset_name}_{safe_arch_name}.pth"
    ...
    def loss_calculation(self, x):
        ...
        ori_mem_x = None
        loss_ce_mem = torch.tensor(0.0, device=self.device) # Initialize loss_ce_mem
        if self.lambda_ce_low_ent > 0:
            ...
            if ori_mem_x is not None:
                ...
                loss_ce_mem = F.cross_entropy(outputs_mem, ori_mem_y.long())
                loss_ce_mem *= self.lambda_ce_low_ent

        loss_CRP = torch.tensor(0.0, device=self.device) # Initialize loss_CRP
        if self.lambda_CRP > 0.0 and ori_mem_x is not None:
            ...
            if "m" in self.class_relation_loss_type or "M" in self.class_relation_loss_type:
                ...
                loss_CRP = mse_loss * self.lambda_CRP
            if "t" in self.class_relation_loss_type or "T" in self.class_relation_loss_type:
                ...
                loss_CRP = topology_loss * self.lambda_CRP
        ###############################################

        total_loss = loss_trg + loss_ce_mem + loss_CRP

        # Store necessary outputs for return
        outputs = {"ensemble": outputs_test + outputs_ema, "student": outputs_test, "teacher": outputs_ema}
    ...
    def forward_and_adapt(self, x):
        ...
        imgs_test = x[0]
        outputs_test = outputs["student"]
        outputs_ema = outputs["teacher"]
        ent_stu = softmax_entropy(outputs_test)
        filter_ids = torch.where(ent_stu < self.e_margin)
        image_low_ent = imgs_test[filter_ids]
        low_ent_logits = outputs_test[filter_ids]
        low_ent_stu = ent_stu[filter_ids]
        pres = outputs_test + outputs_ema
        low_ent_y = pres[filter_ids].argmax(1)
        ...
        self.model_ema = ema_update_model(model_to_update=self.model_ema, model_to_merge=self.model, momentum=self.m_teacher_momentum, device=self.device, update_all=True)
    ...
    def collect_params_vit(self):
        ...
        Walk the model's modules and collect all parameters.
        Return the parameters and their names.
        Note: this version collects all parameters with requires_grad=True.
        """
        params = []
        names = []
        for nm, p in self.model.named_parameters():  # 遍历模型的所有参数
            if p.requires_grad:  # 检查参数是否需要梯度更新
                params.append(p)  # 添加参数到列表
                names.append(nm)  # 添加参数名称到列表
