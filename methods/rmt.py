...
class RMT(TTAMethod):
    def __init__(self, cfg, model, num_classes):
        ...
        if cfg.SOURCE.USE_SYN:
            ...
            # batch_size_src = cfg.OBAO.BUFFER_BS_SIZE
        else:
        ...
        if os.path.exists(fname):
            logger.info("Loading class-wise source prototypes from file...")
            logger.info(f"Prototypes file: {fname}")
        ...
        if "vit" in self.cfg.MODEL.ARCH or "VIT" in self.cfg.MODEL.ARCH:
            self.models = [self.feature_extractor, self.classifier, self.model_ema, self.projector]
        else:
    ...
    def forward_and_adapt(self, x):
        ...
        # if vit, due to the classifier is moved form self.model.model, ema update the classifier
        if "vit" in self.cfg.MODEL.ARCH or "VIT" in self.cfg.MODEL.ARCH:
            self.model_ema.model.head = ema_update_model(
                model_to_update=self.model_ema.model.head,
                model_to_merge=self.classifier,
                momentum=self.m_teacher_momentum,
                device=self.device,
                update_all=True
            )
