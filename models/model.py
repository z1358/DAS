...
from utils.misc import rm_substr_from_state_dict
...
def get_timm_model(model_name: str, cfg=None):
    """
...
def get_model(cfg, num_classes: int, device: Union[str, torch.device]):
    ...
    if cfg.MODEL.USE_CLIP:
        ...
        try:
            ...
            base_model, preprocess = get_torchvision_model(cfg.MODEL.ARCH, weight_version=cfg.MODEL.WEIGHTS) # Compose(Resize(size=[256], interpolation=bilinear, max_size=None, antialias=None)CenterCrop(size=(224, 224))ToTensor())
        except ValueError:
            try:
                # load model from timm
                base_model, preprocess = get_timm_model(cfg.MODEL.ARCH, cfg)
            except ValueError:
                try:
...
def split_up_model(model, arch_name: str, dataset_name: str):
    ...
    if hasattr(model, "model") and hasattr(model.model, "pretrained_cfg") and hasattr(model.model, model.model.pretrained_cfg["classifier"]):
        ...
        # classifier = deepcopy(getattr(model.model, model.model.pretrained_cfg["classifier"])) # ❗No deepcopy here
        classifier_attr = model.model.pretrained_cfg["classifier"]
        classifier = getattr(model.model, classifier_attr)
        encoder = model
