...
class Buffer(nn.Module):
    def __init__(self, args, input_size=None, n_classes=100, MIXED_PRECISION=False):
        super().__init__()
        self.args = args
    ...
    def add_reservoir(self, x, y, ents, logits):
        ...
        self.bx[idx_buffer] = x[idx_new_data].cuda().to(self.bx.dtype)
        self.by[idx_buffer] = y[idx_new_data].cuda().to(self.by.dtype)
        self.ents[idx_buffer] = ents[idx_new_data].cuda().to(self.ents.dtype)
        self.logits[idx_buffer] = logits[idx_new_data].cuda().to(self.logits.dtype)
