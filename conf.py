...
# _C.DATA_SYN_DIR = "/mnt/Data1/zzl/Syn_datasets"

# whether to use paired synthetic data
_C.DATA_Paired = False

# Weight directory
_C.CKPT_DIR = "/mnt/Data1/zzl/projects/ECCV-code/github/ckpt"
...
_C.CONTRAST.BASE_TEMPERATURE = 1.0
...
# --------------------------------- SDA options ----------------------------- #
_C.SDA = CfgNode()
_C.SDA.FINE_TUNED_CHECKPOINT = "/mnt/Data1/zzl/projects/Diffusion-Driven-Test-Time-Adaptation-via-Synthetic-Domain-Alignment-main/finetuned_ckpt/ResNet50.pth"

# --------------------------------- TIPI options ----------------------------- #
_C.TIPI = CfgNode()

# _C.TIPI.use_mean_teacher = False
# _C.TIPI.use_strong_aug = False
# _C.TIPI.use_weak_aug = False
# _C.TIPI.clip = True
# _C.TIPI.aug_num = 1
# _C.TIPI.use_mean_delta = False
_C.TIPI.lr_per_sample = 0.00025 # change this for LR
_C.TIPI.epsilon = 0.00784313725
_C.TIPI.random_init_adv = False
_C.TIPI.reverse_kl = True
# _C.TIPI.symmetric_kl = False
_C.TIPI.tent_coeff = 0.0
_C.TIPI.use_test_bn_with_large_batches = False
...
_C.OBAO.NUM_SAMPLES_WARM_UP = 0  # Number of samples used during the mean teacher warm-up
...
# _C.OBAO.SYN_FILTER = False 
# _C.OBAO.SYN_FILTER_MARGIN = 0.4
_C.OBAO.LAMBDA_CONT = 0.0 
_C.OBAO.SYN_PROTO = False 
_C.OBAO.ONLINE_PROTO_MOMENT = 0.9995
_C.OBAO.LAMBDA_RELATION_TRANSFER = 0.0
_C.OBAO.RELATION_TRANSFER_TEMP = 0.32

#################
_C.OBAO.LAMBDA_D = 0.0              # Lambda for DD loss
_C.OBAO.LAMBDA_OPE = 0.0              # Lambda for DE loss
_C.OBAO.LAMBDA_DISTILL = 0.0          # Lambda for progressive distillation loss    
_C.OBAO.ANCHOR_PROTO_SYN = False
_C.OBAO.USE_PROJECTOR = True        # whether to use projector for the alignment

# --------------------------------- INSAD options ----------------------------- #
_C.INSAD = CfgNode()

_C.INSAD.lr_per_sample = 0.00025 # change this for LR
_C.INSAD.epsilon = 0.00784313725
_C.INSAD.random_init_adv = False
_C.INSAD.reverse_kl = True
# _C.INSAD.symmetric_kl = False
_C.INSAD.tent_coeff = 0.0
_C.INSAD.use_test_bn_with_large_batches = False
_C.INSAD.use_symmetric_kl = False
...
# _C.CMAE.mt_alpha = 0.999
_C.CMAE.rst_m = 0.01
_C.CMAE.block_size = 16
_C.CMAE.mask_ratio = 0.5
_C.CMAE.use_hog = True
_C.CMAE.hog_ratio = 0.5
_C.CMAE.random_mask = False
...
_C.VIDA.alpha_vida = 0.8
_C.VIDA.thr = 0.2
_C.VIDA.warmup_lr = 1e-3
_C.VIDA.NUM_SAMPLES_WARM_UP = 50000
...
_C.RMT.DETECTION = False
_C.RMT.ST_LAYERS = [-1]

# --------------------------------- RMT options ----------------------------- #
_C.STYLE = CfgNode()

_C.STYLE.LAMBDA_CE_SRC = 1.0         # Lambda for source replay. Set to 0 for source-free variant
_C.STYLE.LAMBDA_CE_TRG = 1.0          # Lambda for self-training
_C.STYLE.NUM_SAMPLES_WARM_UP = 0  # Number of samples used during the mean teacher warm-up
_C.STYLE.DETECTION = False
_C.STYLE.ST_LAYERS = [-1]
_C.STYLE.FourierST_RATIO = 0.5
_C.STYLE.FourierST_ALPHA = 1.0
_C.STYLE.USE_FourierST = True
_C.STYLE.LAMBDA_CONT = 0.0            # # Lambda for contrastive learning

# --------------------------------- FREELAUNCH options --------------------------- #
_C.FREELAUNCH = CfgNode()

_C.FREELAUNCH.LAMBDA_CE_TRG = 1.0        # Lambda for self-training
_C.FREELAUNCH.LAMBDA_CE_SRC = 0.5          # Lambda for contrastive learning
_C.FREELAUNCH.NUM_SAMPLES_WARM_UP = 50000
...
_C.SOURCE.USE_SYN = False

# ------------------------------- Testing options ------------------------- #
_C.TEST = CfgNode()

# Number of workers for test data loading
_C.TEST.NUM_WORKERS = 16

_C.TEST.IMG_SIZE = None
...
# 是否根据batch size调整学习率
_C.TEST.LR_DECAY_WITH_BS = False
...
def load_cfg_from_args(description="Config options."):
    ...
    cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{cfg.MODEL.ADAPTATION}/{cfg.MODEL.ARCH}/{cfg.SETTING}/{cfg.CORRUPTION.DATASET}_{current_time}")
...
def complete_data_dir_path(data_root_dir: str, dataset_name: str, is_synthetic: bool = False):
    ...
    if is_synthetic:
        mapping = { # "imagenet": "", # directory containing the 6 splits of "cleaned versions" from http://ai.bu.edu/M3SDA/#dataset
               "cifar10": "synthetic_data_cifar10",       #
               "cifar10_c": "",     # 
               "cifar100": "synthetic_data_cifar100",      # 
               "cifar100_c": "",    # 
               "imagenet": "synthetic_data_imagenet", # 
                "imagenet_c": "",     #
            }
