...
def get_accuracy(model: torch.nn.Module,
    ...
    num_correct_dict = {}
    num_samples = 0
    batch_idx = global_idx  # keep track of the global batch index across multiple domains
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            batch_idx += 1
            imgs, labels = data[0], data[1]
            if cfg.DATA_Paired: # used for DDA or SDA
                syn_imgs = data[-2]
                paired_imgs = torch.cat([imgs, syn_imgs], dim=0)
                output = model(paired_imgs.to(device))
            else:
            ...
            if isinstance(output, dict):
                ...
                for key, value in output.items():
                    ...
                    if key not in num_correct_dict:
                        num_correct_dict[key] = 0.0
                    
                    correct = (predictions == labels.to(device)).float().sum().item()
                    num_correct_dict[key] += correct

                    batch_acc = correct / (imgs[0].shape[0] if isinstance(imgs, list) else imgs.shape[0])
                    batch_metrics[f"accuracy/batch_{key}"] = batch_acc
                if use_wandb:
                    wandb.log(batch_metrics, step=batch_idx)
            else:
                predictions = output.argmax(1)
                if dataset_name == "imagenet_d" and domain_name != "none":
                    mapping_vector = list(IMAGENET_D_MAPPING.values())
                    predictions = torch.tensor([mapping_vector[pred] for pred in predictions], device=device)
                if 'default' not in num_correct_dict:
                    num_correct_dict['default'] = 0.0
                num_correct_dict['default'] += (predictions == labels.to(device)).float().sum().item()

                correct = (predictions == labels.to(device)).float().sum().item()
                batch_acc = correct / (imgs[0].shape[0] if isinstance(imgs, list) else imgs.shape[0])
                if use_wandb:
                    wandb.log({"accuracy/batch_default": batch_acc}, step=batch_idx)
    ...
    accuracy_dict = {key: num_correct / num_samples for key, num_correct in num_correct_dict.items()}
    return accuracy_dict, domain_dict, num_samples, batch_idx
